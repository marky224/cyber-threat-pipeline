...
Project is **feature-complete** against the seven specs in `_private/specs/`. The weekly cron writes a full `pipeline.runs` audit row, the Evidence site refreshes, the Grafana dashboard shows the live operational view, and the analyst brief is regenerated by Claude Sonnet 4.6 each run.
